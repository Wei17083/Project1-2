...
public class SolarSystem implements ODESolverInterface, ODEFunctionInterface {
        ...
        public RateInterface call(double t, StateInterface y) {
                ...
                for (int i = 0; i < bodies.length; i++) {
                        rate.addPositionChange(y1.getVelocityList().get(bodies[i].getID()));
                        ArrayList<Vector3dInterface> forces = new ArrayList<>();
                        for (Body b2 : bodies) {
                                if (!(bodies[i].equals(b2))) {
                                        forces.add(b2.gravitationalPull(bodies[i]));
                                }
                        }
                        rate.addVelocityChange(VectorTools.sumAll(forces).mul(1 / bodies[i].getMass()));
                }
                return rate;
        }
