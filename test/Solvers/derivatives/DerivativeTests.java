...
class DerivativeTests {
    ...
    @Test
    void testDerivativePositionGetAllDerivatives() {
        SolarSystem system = new SolarSystem(bodies);
        ArrayList<Vector3dInterface> positionChangesCorrect = system.call(0, system.getInitialState()).getPositionChanges();
        Derivative positionDerivative = new DerivativePosition(system.getInitialState().getVelocityList());
        ArrayList<Vector3dInterface> positionDerivativeToTest = positionDerivative.getAllDerivatives();
        for (int i = 0; i < positionChangesCorrect.size() ; i++) {
            assertTrue(VectorTools.equals(positionChangesCorrect.get(i), positionDerivativeToTest.get(i)));
        }
    }

    @Test
    void testDerivativeVelocityGetAllDerivatives() {
        final double ACCURACY = 1E-8;
        SolarSystem system = new SolarSystem(bodies);
        ArrayList<Vector3dInterface> velocityChangesCorrect = system.call(0, system.getInitialState()).getVelocityChanges();
        Derivative velocityDerivative = new DerivativeVelocity(system.getInitialState().getPositionList(), system.getBodies());
        ArrayList<Vector3dInterface> velocityDerivativeToTest = velocityDerivative.getAllDerivatives();
        for (int i = 0; i < velocityChangesCorrect.size() ; i++) {
            Vector3dInterface accelerationToTest = velocityDerivativeToTest.get(i);
            Vector3dInterface accelerationCorrect = velocityChangesCorrect.get(i);
            assertEquals(accelerationCorrect.getX(), accelerationToTest.getX(), Math.abs(ACCURACY*accelerationToTest.getX()) );
            assertEquals(accelerationCorrect.getY(), accelerationToTest.getY(), Math.abs(ACCURACY* accelerationToTest.getY()) );
            assertEquals(accelerationCorrect.getZ(), accelerationToTest.getZ(), Math.abs(ACCURACY* accelerationToTest.getZ()) );
    ...
    @Test
    void testGravitation() {
        SolarSystem system = new SolarSystem(bodies);
        Body[] bodyList = system.getBodies();
        final double ACCURACY = 1E-8;
        for (Body b1: bodyList) {
            for (Body b2: bodyList) {
                if(!(b1.equals(b2))) {
                    Vector3dInterface gravForce = GravitationalForce.gravitationalForce(b1.getPosition(), b2.getPosition(), b1.getMass(), b2.getMass());
                    Vector3dInterface gravPullCorrect = system.gravitationalPull(b1.getMass(), b2.getMass(), b1.getPosition(), b2.getPosition() );
                    assertEquals(gravForce.getX(), gravPullCorrect.getX(), Math.abs(ACCURACY*gravForce.getX()) );
                    assertEquals(gravForce.getY(), gravPullCorrect.getY(), Math.abs(ACCURACY* gravForce.getY()) );
                    assertEquals(gravForce.getZ(), gravPullCorrect.getZ(), Math.abs(ACCURACY* gravForce.getZ()) );
                }
            }
        }
